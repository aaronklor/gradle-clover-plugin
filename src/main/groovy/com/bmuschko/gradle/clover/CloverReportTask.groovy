...
abstract class CloverReportTask extends DefaultTask {
    ...
    /**
     * The directory to check for test results instead of using the results generated by junit rather than clover.
     */
    File testResults

    /**
     * The ant-blob include pattern to select test results from {@link #testResults}. Default: {@value}.
     */
    ...
    private void writeReport(File outfile, ReportType reportType, String filter) {
        ant."clover-report"(initString: "$project.buildDir/${getInitString()}") {
            current(outfile: outfile, title: project.name) {
                ...
                if (testResults && testResultsInclude) {
                    testresults(dir: testResults) {
                        include(name: testResultsInclude)
                    }
                }
            }
        }
